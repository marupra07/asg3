class Camera{
    ...
    //rotate for cursor movement
    rotateLookAt(deltaX, deltaY) {
        var viewDir = this.at.sub(this.eye);
    
        var rotationMatrixY = new Matrix4();
        rotationMatrixY.setRotate(deltaX, 0, 1, 0);
        var newViewDir = rotationMatrixY.multiplyVector3(viewDir);
    
        var rightAxis = Vector3.cross(newViewDir, this.up);
        rightAxis.normalize();
        var rotationMatrixX = new Matrix4();
        rotationMatrixX.setRotate(deltaY, rightAxis.elements[0], rightAxis.elements[1], rightAxis.elements[2]);
        newViewDir = rotationMatrixX.multiplyVector3(newViewDir);
    
        this.at = this.eye.add(newViewDir);
    } 

    zoom(factor) {
        var viewDir = this.at.sub(this.eye);
        var scaledDir = viewDir.mul(factor);
